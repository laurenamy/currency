...
contract('Contribution', function (accounts) {
  ...
  describe("sendContribution", async function () {
    it("should revert if the amount is less than or equal to zero.", async function () {
      await expectRevert(contributionInstance.sendContribution(tokenOwner, { from: donor, to: tokenOwner, value: 0 }), "Amount must not be zero.");
    });
    it("should revert if the amount of tokens is greater than the supply", async function () {
      await expectRevert(contributionInstance.sendContribution(tokenOwner, { from: donor, value: largeAmountEth }), "Insufficient amount of tokens.");
    });
    it("emits a Sent event on successful contribution", async function () {
      const donorTokens = await tokenInstance.balanceOf(donor);
      console.dir("donor balance");
      console.dir(await web3.eth.getBalance(donor));
      console.dir(donorTokens + " tokens");

      const ownerTokens = await tokenInstance.balanceOf(tokenOwner);
      console.dir("owner balance");
      console.dir(await web3.eth.getBalance(tokenOwner));
      console.dir(ownerTokens + " tokens");

      const { logs } = await contributionInstance.sendContribution(tokenOwner, { from: donor, value: amountEth });
